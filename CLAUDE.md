...
- **Cron is timezone-aware (`Asia/Jerusalem`).** The trigger uses EventBridge Scheduler with a timezone, so the cron is the local wall-clock time — `10:00` means 10:00 Israel time. Just write the time you want; no UTC conversion.
