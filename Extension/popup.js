...
class WoltFlowTokenReviewer {
  ...
  addEventListeners() {
    document.getElementById('copyAccess').addEventListener('click', () => this.copyToken('access'));
    document
  ...
  async getCookie(cookieName, tokenType) {
    ...
    try {
      ...
      const domains = ['.wolt.com', 'wolt.com', 'consumer-api.wolt.com', 'restaurant-api.wolt.com'];
      ...
      for (const domain of domains) {
        ...
        if (cookies && cookies.length > 0) {
          ...
          if (cookieName === this.accessTokenName && decodedValue.startsWith('{')) {
            try {
  ...
  displayToken(type, result) {
    const element = document.getElementById(type === 'access' ? 'accessToken' : 'refreshToken');
    const copyBtn = document.getElementById(type === 'access' ? 'copyAccess' : 'copyRefresh');
